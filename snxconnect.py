...
class HTML_Requester (object) :
    ...
    def login (self) :
        ...
        if errorMessage :
            ...
            if 'Confirmation code' not in errorMessage.string :
            	if 'Additional authentication required' not in errorMessage.string :
                    return
    ...
    def parse_pw_response (self) :
        ...
        for input in form.find_all ('input') :
            if input.attrs.get ('type') == 'text' : #filtra somente tipo text para pegar todos os campos
                continue
            if 'name' not in input.attrs :
                continue
            if input ['name'] in ('SendMethod', 'passwordDisplayed', 'phoneNumbersSelection', 'btnCancel') :
                continue
...
def main () :
    ...
    cmd.add_argument \
        ...
        , default = cfg.get ('multi_challenge', True)
        )
    cmd.add_argument \
        ( '-P', '--password'
    ...
    cmd.add_argument \
        ...
        , default = cfg.get ('realm', 'ssl_vpn_Username_Password')
        )
    cmd.add_argument \
        ( '-s', '--save-cookies'
